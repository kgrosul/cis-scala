...
object task2 extends App {
  ...
  def traverse[A](list: List[Option[A]]): Option[List[A]] = {
    if (list.forall(_.nonEmpty)) Some(list.map(_.get))
    else None
  }
  ...
  def splitEither[A, B](list: List[Either[A, B]]): (List[A], List[B]) = {
    val left = list.collect { case Left(value) => value }
    val right = list.collect { case Right(value) => value}
    (left, right)
  }
